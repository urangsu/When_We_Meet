...
### 5. 다음 작업
1. **task.md 로드맵 정리**
   - 현재 진행 상태(Phase D-2) 기준으로 `task.md` 로드맵 섹션 최신화
   - 지연되거나 남은 작업들을 미완료 목록으로 재배치

2. **DB 연동 전 데이터 모델 및 스키마 확정**
   - Meeting / MeetingResponse / ConfirmedPlan / InviteLink 모델 간의 관계 정리
   - Supabase 또는 Firebase 연동을 위한 테이블/컬렉션 설계

3. **실제 초대 링크 응답 흐름 설계**
   - `link_anyone` 정책 구현 및 유효성 검사(maxResponses, expiresAt) 설계
   - Guest Web 응답을 Host Dashboard로 연결하는 실제 데이터 파이프라인 구조 설계

4. **ConfirmedShareScreen 개선**
   - 확정된 모임 정보(날짜/시간/장소/활동) 카드 UI 고도화
   - 실제 공유 UX 전 단계의 Prototype 카드 UI 개선
