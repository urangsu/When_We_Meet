...
export const DashboardScreen = ({ onBack, onNext }: { onBack: () => void, onNext: () => void }) => {
  const stats = [
    { label: '갈게요', count: 5, color: 'text-success', bg: 'bg-success-bg' },
    { label: '아마도', count: 2, color: 'text-warning', bg: 'bg-warning-bg' },
    { label: '어려움', count: 1, color: 'text-danger', bg: 'bg-danger-bg' },
    { label: '미응답', count: 2, color: 'text-ink-hint', bg: 'bg-ink-line' },
  ];
    ...
    <div className="flex flex-col gap-8 h-full">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="hero-title text-2xl">응답 현황</h1>
        </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {stats.map((stat) => (
          <div key={stat.label} className={`${stat.bg} rounded-2xl p-3 flex flex-col items-center gap-1`}>
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          {rankings.map((rank, i) => (
            <div key={rank.date} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-ink-line">
              <div className={`
                w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm
                ${rank.best ? 'bg-warning-bg text-warning' : 'bg-ivory text-ink-hint'}
              `}>
      ...
      <Card className="bg-rose-light border-rose/10 flex flex-col gap-3">
      ...
      <div className="mt-auto pb-10">
        <Button onClick={onNext} size="full">이 날짜로 확정하기</Button>
      </div>
    </div>
  );
};
